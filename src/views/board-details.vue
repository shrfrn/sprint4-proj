<template>
  <section v-if="board" class="board-details">
    ...
    <task-list
      :tasks="board.groups[1].tasks"
      :color="board.groups[1].style"
      :groupIdx="idx"
    />
...
export default {
  ...
  computed: {
    delegates() {
      return this.$store.getters.currBoard.groups[0].tasks[0].columns[
        "delegates"
      ];
    },
    status() {
      console.log(
        this.$store.getters.currBoard.groups[0].tasks[0].columns["status"]
      );
